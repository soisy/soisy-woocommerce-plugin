<?php
    ...
    global $unset;
	$unset = -1;
	foreach ( $active_plugins as $k => $plugin ) {
		if ( $plugin == 'woocommerce/woocommerce.php' ) {
			$check_wc = true;
		}
		if ( $plugin == 'soisy-pagamento-rateale/soisy-woocommerce-plugin.php' ) {
			$unset = $k;
		}
	}
	if ( empty( $check_wc ) ) {
	...
	function init_soisy()
	{
		...
		if ( $unset > - 1 ) {
			if ( ! class_exists( 'WooCommerce\WC_Payment_Gateway' ) && false ) {
				//do_action( 'qm/debug', 'class not exists' );
				add_filter( 'active_plugins', function ( $plugins ) use ( $unset ) {
					unset( $plugins[ $unset ] );
				...
				add_action('admin_notices', 'nogateway__notice__error');
				function nogateway__notice__error() {
					printf('<div class="notice notice-error"><p>%s</p></div>',
						__('Unfortunately Soisy Gateway is not compatible with your installation.', 'soisy')
					);
				}
